...
//update metering mapping
routes.post(
  '/v1/account/metering/:resource_id',
  function *(req) {
    debug('updating metering  plan mapping %s',
      req.params.resource_id);
debug('new mapping added is %s',JSON.stringify(req.body.mapping));
var clone = JSON.parse(JSON.stringify(sampleMetering));
	clone[req.params.resource_id] = req.body.mapping;
debug("before update %s",JSON.stringify(sampleMetering));
 sampleMetering = clone;
debug('updated mapping is %s',JSON.stringify(sampleMetering));
    return {
      status: 201,
       body : sampleMetering
    };
...
//update rating mapping
routes.post(
  '/v1/account/rating/:resource_id',
  function *(req) {
    debug('updating metering  plan mapping %s',
      req.params.resource_id);
debug('new mapping added is %s',JSON.stringify(req.body.mapping));
var clone = JSON.parse(JSON.stringify(sampleRatings));
	clone[req.params.resource_id] = req.body.mapping;
debug("before update %s",JSON.stringify(sampleRatings));
 sampleRatings = clone;
debug('updated mapping is %s',JSON.stringify(sampleRatings));
    return {
      status: 201,
       body : sampleRatings
    };
...
//update pricing mapping
routes.post(
  '/v1/account/pricing/:resource_id',
  function *(req) {
    debug('updating metering  plan mapping %s',
      req.params.resource_id);
debug('new mapping added is %s',JSON.stringify(req.body.mapping));
var clone = JSON.parse(JSON.stringify(samplePricings));
	clone[req.params.resource_id] = req.body.mapping;
debug("before update %s",JSON.stringify(samplePricings));
 samplePricings = clone;
debug('updated mapping is %s',JSON.stringify(samplePricings));
    return {
      status: 201,
       body : samplePricings
    };
